...
public static class RectangleFExtensions
{
    ...
    /// </returns>
    public static RectangleF[] DeflateInOne(this IEnumerable<RectangleF> rects) => rects.Select(r => r.DeflateInOne()).ToArray();
    ...
    {
        var length = orientation.IsVerticalOrientation()
            ? (int) (rect.Height * lenghtPercent)
            : (int) (rect.Width * lenghtPercent);

        var restRect = RectangleF.Empty;
        var lengthRect = RectangleF.Empty;
